"""Argon gas properties for thermoacoustic calculations."""
...
class Argon(Gas):
    ...
    Argon is a monatomic ideal gas with γ = 5/3. This class provides
    ...
    Viscosity correlation:
        μ(T) = 2.27e-5 * (T/300)^0.85 Pa·s

    Thermal conductivity correlation:
        κ(T) = 0.0177 * (T/300)^0.85 W/(m·K)

    Examples
    --------
    >>> ar = Argon(mean_pressure=101325.0)
    >>> ar.density(T=300.0)  # Density at 300 K and 1 atm
    1.622...
    >>> ar.sound_speed(T=300.0)  # Sound speed at 300 K
    322.6...
    """

    # Reference values for transport property correlations
    _T_REF: float = 300.0  # Reference temperature (K)
    _MU_REF: float = 2.27e-5  # Reference viscosity at T_ref (Pa·s)
    _KAPPA_REF: float = 0.0177  # Reference thermal conductivity at T_ref (W/(m·K))
    ...
    def molar_mass(self) -> float:
        ...
        -------
        float
            Molar mass in kg/mol (39.948e-3).
        """
        return MOLAR_MASS["argon"]

    def density(self, T: float, P: float | None = None) -> float:
        """
        Calculate argon density using ideal gas law.
    ...
    def sound_speed(self, T: float, P: float | None = None) -> float:
        """
        Calculate sound speed in argon.
    ...
    def gamma(self, T: float, P: float | None = None) -> float:
        """
        Return ratio of specific heats for argon.
    ...
    def prandtl(self, T: float) -> float:
        ...
        For argon, the Prandtl number is nearly constant at approximately 0.67
